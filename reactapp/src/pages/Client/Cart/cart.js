...
import ConfirmCartModal from "./ConfirmDeleteCartModal"
...
const Cart = () => {
  ...
  const confirmDelete = () => {
    if (itemToDelete) {
      handleUpdateCart(itemToDelete.id, "delete")
    }
  }
  ...
  const handleCheckout = async () => {
    ...
    const responseCreateBill = await createBillForUser({
      ...
      totalCost: (Number.parseFloat(calculateTotal()) + 5).toFixed(2),
      shippingMethod: "GHTK",
        ...
        <div className="cart-table-container">
          <div className="select-all-container">
            <label className="checkbox-container">
              <input type="checkbox" checked={isAllSelected} onChange={(e) => selectAllItems(e.target.checked)} />
                    ...
                    <td>
                      <div className="quantity-control">
                        <button
                          ...
                          onClick={() => handleUpdateCart(item.id, "down")}
                        >
                          -
                        </button>
                        ...
                        <button
                          className="quantity-btn"
                          disabled={item.quantity + 1 >= item.Book.stock}
                          onClick={() => handleUpdateCart(item.id, "up")}
                        >
                          +
                        </button>
                      </div>
          ...
          <div className="checkout-container">
            ...
            <div className="checkout-summary">
              <div className="selected-items">
                ...
                <div className="cart_selected-items-list">
                  {getSelectedItems().length > 0 ? (
                    getSelectedItems().map((item) => (
                      <div key={item.id} className="cart_selected-item">
                    ...
                    <div className="cart_empty-selection">No items selected</div>
                  )}
                </div>
              </div>
                ...
                <div className="payment-options">
                  {paymentMethods.length > 0 ? (
                    paymentMethods.map((item) => (
                      <div className="payment-option" key={item.id}>
              ...
              <div className="checkout-total">
                ...
                <div className="total">
                  <span>Total:</span>
                  <span>${(Number.parseFloat(calculateTotal()) + 5).toFixed(2)}</span>
                </div>
              </div>
            </div>
